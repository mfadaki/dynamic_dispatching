...
class ALP:
    ...
    # ── feature matrix ────────────────────────────────────────────────────────
    def _phi_raw(self, s):
        """Raw (unnormalized) basis matrix. Return (B,) or (H, B).
        Columns: [const, depot_age1, depot_age2, depot_age3,
                  lab_shortfall, imbalance, exp_risk].
        """
        ...
        if sc:
            s = s[None, :]
        r = np.c_[
            self.phi_const(s)[:, None],
            self.phi_depot_age(s, 0)[:, None],
            self.phi_depot_age(s, 1)[:, None],
            self.phi_depot_age(s, 2)[:, None],
    ...
    def E_phi_next(self, mdp, s, a, exog):
        ...
        H, N = s.shape[0], exog.shape[0]
        sn   = mdp.transition(s, a, exog)
        return self.phi(sn.reshape(H * N, N_STATE)).reshape(H, N, self.B).mean(1)
