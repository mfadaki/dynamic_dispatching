...
=======================================
ISOLATED COPY of classes/mdp.py, used ONLY by evaluate_approximation.py.

Identical to the production classes/mdp.py except for three fixes needed
specifically for the EXACT, exhaustive-enumeration LP comparison to work
(none of these matter for the production training pipeline, which never
enumerates the full state space):

  1. apply_action_and_event(): dispatch is now clipped at N_MAX, matching
     the arrival step's own clipping. Without this, a lab already near
     capacity receiving a dispatch from a near-full depot can exceed
     N_MAX, which breaks state enumeration (the state space is assumed
     bounded by N_MAX everywhere else in the codebase).
  2. _transition_single(): the end-of-day trigger now uses tau_new<=1e-6
     instead of tau_new<=0, matching the tolerance the manuscript itself
     specifies ("decrements to zero, or below a tolerance") -- the strict
     version can fail on floating-point residuals under exact enumeration.
  3. cost(): the same tolerance applied to its EOD/expiry gate, to stay
     consistent with fix #2 above.

Deliberately kept as a SEPARATE file rather than edited in place, so the
production classes/mdp.py -- and therefore every existing/future training
run through main.py -- is completely unaffected. Do not import this
module from anything other than evaluate_approximation.py.

------------------------------------------------------------------------
Original classes/mdp.py docstring follows, unmodified:
------------------------------------------------------------------------
...
class MDP:
    ...
    def apply_action_and_event(n, a, event):
        ...
        if a > 0:
            ...
            # Clipped at N_MAX, matching the arrival step's own clipping
            # below: without this, the state space is not actually bounded
            # by N_MAX, only usually bounded in practice at large N_MAX
            # under typical loads (a lab already near capacity receiving a
            # dispatch from a near-full depot can otherwise exceed N_MAX).
            n[p_lab + 1] = np.minimum(n[p_lab + 1] + n[0], N_MAX)
            n[0, :]       = 0.0         # empty depot
    ...
    def _cost_single_raw(self, s, a):
        ...
        # Dispatch-overflow penalty: apply_action_and_event's dispatch step
        # clips n[lab] at N_MAX, silently discarding any depot kits that
        # don't fit. Without a cost here, discarding is FREE -- and the
        # exact optimal policy was found to exploit this: dispatching
        # surplus depot inventory into an already-(near-)full lab can be
        # cheaper than actually delivering it, since delivered kits go on
        # to accrue real future holding/expiry costs while discarded ones
        # simply cease to exist. Billed at the depot expiry rate, since a
        # discarded kit is at least as wasteful as one left to expire.
        if a > 0:
            overflow = np.maximum(0., (n[a] + n[0]) - N_MAX).sum()
            c += C_EXP_DEPOT * overflow
