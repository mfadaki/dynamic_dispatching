...
def lp_warm_start(mdp, alp, n_init, n_exog, theta_lb=None, theta_ub=None, run_seed=0):
    """Solve a sampled ALP for an initial θ. Initialisation only — the LP
    solution does not floor or ceiling the PSMD search.

    run_seed offsets the internal seed (default run_seed=0 reproduces the
    exact previous behaviour, seed=0) rather than hardcoding it, so a
    caller can get an independently-reproducible warm start by passing a
    different run_seed -- see run_psmd's docstring for why this needs to
    move together with the other two hardcoded seeds in this module, not
    change on its own."""
    np.random.seed(1000 * run_seed + 0)
...
def _compute_break_even_floor(mdp, alp, theta_ref, exog, feature_idx):
    ...
    FLOOR IS NOW SELF-SUFFICIENT (worst-case σ_others=0), not "others at
    warm-start value":
        θ_b^floor = max(0, c_min / Δφ_b)

    Previously this assumed the other break-even features would hold near
    their warm-start (theta_ref) values for the rest of training:
        θ_b^floor = max(0, (c_min − σ_others) / Δφ_b),  σ_others from theta_ref
    That guarantee is only valid at t=0. Empirically, across multiple runs
    and recalibrations, theta_4/5/6 (lab_shortfall/imbalance/exp_risk) all
    decline together throughout training rather than holding steady near
    their warm-start values -- so σ_others erodes over time even though
    each individual θ_b individually respects its OWN floor, and the joint
    inequality the floor was meant to guarantee (Σθ_b·Δφ_b > c_min) can
    fail later in training even with no single floor violated. This showed
    up as PSMD training collapsing to an always/almost-always-hold policy
    (dispatch frequency -> 0, performance below the myopic baseline) on
    several different calibrated instances, not just one unlucky seed.

    Assuming σ_others=0 (this feature must clear c_min on its own, with NO
    help from any other feature) removes that dependency entirely: even if
    every other break-even-indexed coefficient decays to zero, THIS floor
    alone still keeps dispatch worthwhile at s*. This is more conservative
    (larger floors, so θ is pinned somewhat higher than the original,
    optimistic computation would allow) -- a deliberate trade-off given
    that "dispatch essentially never happens" has been the far more severe
    and more frequently observed failure mode than "dispatch fires
    somewhat more often than the theoretically optimal threshold."
    ...
    All quantities come from problem parameters — no magic numbers, and
    (unlike the previous version) no dependence on any OTHER feature's
    theta value at all, warm-start or otherwise.
    """
    ...
    B = alp.B
    # Kept only for the printed comparison below -- no longer used to
    # compute the returned floor itself.
    sigma_others_at_warmstart = sum(
        theta_ref[b] * delta_phi[b] for b in range(B) if b != feature_idx
    )
    floor_optimistic = max(0.0, (c_min - sigma_others_at_warmstart) / delta_phi[feature_idx])
    floor = max(0.0, c_min / delta_phi[feature_idx])   # self-sufficient (sigma_others=0)
    print(f"  break-even floor θ_{feature_idx}: {floor:.4f}  "
          f"(Δφ={delta_phi[feature_idx]:.4f}, c_min={c_min:.1f}, "
          f"self-sufficient; would have been {floor_optimistic:.4f} under the "
          f"old warm-start-dependent computation, σ_others={sigma_others_at_warmstart:.4f})")
    return floor
...
def run_psmd(mdp, alp, lb_calc, ub_calc, cfg, run_seed=0):
    ...
    points for the diagnostic LB) as one unit, via a run_seed*1000 offset on
    each of the three internal seeds -- NOT by changing any one of them in
    isolation, since they're used together to build a single coherent
    training run and offsetting just one would leave the others identical
    across "different" seeds, silently defeating the point of varying it.
    run_seed=0 (the default) reproduces the exact previous hardcoded
    behaviour (seeds 0, 1, 2) byte-for-byte.

    NORM_SEED (feature/cost normalization) deliberately does NOT move with
    run_seed -- normalization defines the feature scale itself, and should
    stay fixed across seeds so results from different seeds remain
    comparable on the same units, rather than each seed getting its own
    slightly different rescaling.
    """
    ...
    for t in range(T):
        ...
        # Sample (s,a) ~ y_t — BEFORE this iteration's dual update — matching
        # Algorithm 1 Step 3, which samples from y_t (not y_{t+1}) to produce
        # theta_{t+1}.
        ...
        theta_t = theta.copy()                        # theta_t, needed below
        theta   = primal.update(theta, grad_vec, eta)  # -> theta_{t+1}

        # Dual update AFTER: Algorithm 1 Step 4 uses theta_t (not
        # theta_{t+1}) to advance y_t -> y_{t+1}.
