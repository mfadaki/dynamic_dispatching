...
# const + L_AGE depot-age + aggregate lab_shortfall + N_LABS per-lab
# shortfall + imbalance + exp_risk + imbalance*exp_risk interaction.
# (+5, not +4: classes/alp_evaluate_approximation.py's _phi_raw always
# appends the imbalance*exp_risk interaction term for N_LABS>=2 as of a
# few experiments back -- this config predated that and would otherwise
# declare one fewer feature than _phi_raw actually produces, causing a
# reshape error identical to the one worked through earlier this session.)
NO_BASIS_FN = L_AGE + N_LABS + 5
_B = NO_BASIS_FN
THETA_LB = [None] + [0.0] * (_B - 1)
THETA_UB = [None] + [50.0] * (_B - 1)
# Interaction term (last feature, index _B-1): allow negative, and exclude
# from the break-even floor mechanism -- same reasoning as
# inputs_evaluate_approximation_L2_interaction.py: its usefulness is
# specifically in going negative (discounting the imbalance brake when
# exp_risk is elevated), and the break-even floor logic assumes a feature
# should be floored at >=0, which doesn't apply here.
THETA_LB[_B - 1] = None
THETA_BREAK_EVEN_IDX = [b for b in range(1, NO_BASIS_FN) if b != _B - 1]
