...
L_AGE       = 3
TAU_MAX     = 8.0
LAMBDA_AGE  = np.array([0.5, 1.0, 1.5])
...
# ── Cost structure ────────────────────────────────────────────────────────────
# Dispatch (order) cost: lowered and identical across labs.
#   Lab selection is still well-defined via the throughput rates μ=[2.0, 2.5]
#   (a dispatch to the faster lab clears faster), not via a cost spread.
C_DISPATCH  = np.array([2.0, 2.0])      # was [5.0, 7.0]

# Holding cost h_{p,a}: depot STRICTLY exceeds both labs for EVERY age class,
# and the two labs are IDENTICAL.
